...
async function main(): Promise<void> {
  ...
  // Thursday runs override env config: 3 golfers, 9 holes, 5:30–5:40 PM ET window.
  // All other days use the env-configured values.
  const nyWeekday = new Intl.DateTimeFormat('en-US', {
    timeZone: 'America/New_York',
    weekday: 'short',
  }).format(new Date());
  const thursdayOverride = nyWeekday === 'Thu';

  const opts: BookerOptions = {
    email,
    password,
    dryRun: parseBool(process.env.DRY_RUN, true),
    targetTimeMin: thursdayOverride ? '17:30' : normalizeTime(process.env.TARGET_TIME_MIN, '10:00'),
    targetTimeMax: thursdayOverride ? '17:40' : normalizeTime(process.env.TARGET_TIME_MAX, '15:00'),
    golfers: thursdayOverride ? 3 : parseInt10(process.env.GOLFERS, 2),
    holes: thursdayOverride ? 9 : parseInt10(process.env.HOLES, 18),
  ...
  log.info('boot.config', {
    ...
    thursday_override: thursdayOverride,
