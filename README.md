...
Cron fires at `50 8 * * 6` UTC = 3:50 AM EST (winter) / 4:50 AM EDT (summer). The script then idles until exactly 5:00:00 AM in `America/New_York` before firing the booking click — DST is handled by the script, not the cron schedule. 10-minute buffer absorbs Railway cron startup jitter.
