...
4. Settings → confirm the cron schedule shows `50 8 * * 6` (UTC).
5. Trigger one manual deploy to verify the build succeeds.

Cron fires at `50 8 * * 6` UTC every Saturday = 3:50 AM EST (winter) / 4:50 AM EDT (summer). The run targets the following Saturday (today + 7). The script idles until exactly 5:00:00 AM in `America/New_York` before firing the booking click — DST is handled by the script, not the cron schedule. 10-minute buffer absorbs Railway cron startup jitter.
